...
export class QueryClientImpl implements Query {
  ...
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("auranw.aura.smartaccount.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  }
  generateAccount(request: QueryGenerateAccountRequest): Promise<QueryGenerateAccountResponse> {
    const data = QueryGenerateAccountRequest.encode(request).finish();
    const promise = this.rpc.request("auranw.aura.smartaccount.Query", "GenerateAccount", data);
...
export const createRpcQueryExtension = (base: QueryClient) => {
  ...
  return {
    ...
    generateAccount(request: QueryGenerateAccountRequest): Promise<QueryGenerateAccountResponse> {
      return queryService.generateAccount(request);
    }
  };
};