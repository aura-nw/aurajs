...
export class AuraWasmClient {
  ...
  public static async connect(endpoint: string): Promise<AuraWasmClient> {
    const tmClient = await Tendermint34Client.connect(endpoint);
    return new AuraWasmClient(tmClient);
  }
  ...
  public async searchTx(query: SearchTxQuery, filter: SearchTxFilter = {}): Promise<readonly IndexedTx[]> {
    ...
    if (isSearchByHeightQuery(query)) {
      txs =
        query.height >= minHeight && query.height <= maxHeight ? await this.txsQuery(`tx.height=${query.height}`) : [];
